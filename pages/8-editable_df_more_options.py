...
with st.expander("Add new columns to the dataframe?"):
    st.write("""To add a new column to the dataframe, fill the fields below and click on the button.
...
new_col_required = add_sel and col_name != "" and col_name not in st.session_state.current_df.columns
if add_sel and col_name != "" and col_name not in st.session_state.current_df.columns:
    st.session_state.current_df[col_name] = [default_value for _ in range(len(st.session_state.current_df))]
    #st.session_state.current_df = initial_df
...
agg_sel = c2.selectbox("Type of aggregation on graph", ["sum", "min", "max", "count", "distinct"])
